...
let
  ...
  inherit (lib)
    concatStringsSep optional toLower forEach types mkOption;
...
in {
  ...
  config = {
    # Include rules in regular system
    services.udev.packages = optional (length cfg != 0) udevRules;
    
    # Include rules in initramfs for early boot
    boot.initrd.services.udev.packages = optional (length cfg != 0) udevRules;
  };
}
